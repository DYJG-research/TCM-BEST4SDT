...
- 🧩 **任务类型与来源**
  - **中医辨证论治任务(TCM_SDT.json)**：来源于临床病例与名家医案；
  - **通用评测任务**：涵盖中医基础知识、医学伦理与大模型内容安全：
    - **中医基础知识(Basic_Knowledge.json)**：选取自公开考试题库，包括国家执业医师资格考试、硕士研究生招生考试临床医学综合能力（中医综合）、中药学职称考试等；
    - **医学伦理(Medical_Ethics.json)**：除来源于权威题库外，亦由专家基于特定场景进行人工标注；
    - **大模型内容安全(LLM_Content_Safety.json)**：由相关领域专家结合实际需求独立设计与构建。
> 注：TCM-BEST4SDT.json是包含以上所有题目的完整数据集。
...
- **CoT评测受“推理轨迹可见性”影响**：我们将“显式输出 CoT”的模型与“不显式输出 CoT”的模型分别报告。
