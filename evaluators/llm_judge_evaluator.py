...
使用Qwen3-32B模型对煎服方法、注意事项、CoT内容完备性进行评分
"""
...
class LLMJudgeEvaluator:
    ...
    def __init__(self, api_host: str, api_port: int, model_name: str = "qwen", api_key: Optional[str] = None):
        """
        初始化LLM判分评测器
    ...
    def evaluate_all(self, case: Dict[str, Any], model_interface, pbar: tqdm, syndrome_choice: str = None, prescription_herbs: str = None, treatment_principles: str = None, skip_think: bool = False) -> Tuple[Dict[str, float], Dict[str, str]]:
        """
        分别调用4次待测模型生成各维度内容，然后调用评分模型完成四个维度评分。
        返回：(scores, responses)
        - scores：各维度得分
        - responses：各维度的原始生成（用于留档）
        - syndrome_choice：证型的具体内容（如"虚实夹杂证;风邪外袭;肝热内郁"），用于注意事项和随症加减评分（若为空，则回退到 case['output']['证型']）
        ...
        # 2. 煎服方法生成 - 优先使用传入参数；未提供时回退到数据集标准内容
        pbar.write("  生成煎服方法 ...")
        ...
        # 3. 注意事项和随症加减合并生成 - 优先使用传入参数；未提供时回退到数据集标准内容
...
"""
        ...
        try:
            ...
            # 优先：适配支持 reasoning_content 的服务（如 gpt-oss-20b）
            try:
            ...
            if "## Thinking" in response and "## Final Response" in response:
                ...
                if thinking_start != -1 and final_response_start != -1 and thinking_start < final_response_start:
                    ...
                    # 如果标签位置不正确，返回空字符串
                    logger.warning("Huatuo-7B标签位置不正确")
                    return ""
            else:
                # 仅存在 </think>（无 <think>）的异常情况：返回其前面的内容作为think
    ...
    def _generate_cooking_method(self, case: Dict[str, Any], prescription_herbs: str, model_interface, treatment_principles: Optional[str] = None) -> str:
        ...
        prompt = f"""\\no_think 你是一位中医专家，请根据以下病例、处方组成和治则治法，制定详细的煎服方法。
...
"""
        ...
        try:
            ...
            # 优先兼容 Huatuo-7B 风格：返回 ## Final Response 之后的正文
            if "## Final Response" in response:
                final_response_start = response.find("## Final Response")
                return response[final_response_start + len("## Final Response"):].strip()
            # 兼容 <think> ... </think> 风格：提取 think 标签外的内容（尽管 clean_think=True 通常已移除）
    ...
    def _generate_precautions_and_modifications(self, case: Dict[str, Any], syndrome_choice: str, prescription_herbs: str, model_interface) -> Dict[str, str]:
        ...
        prompt = f"""\\no_think 你是一位中医专家，请根据以下病例、证型诊断和处方组成，制定随症加减方案并给出注意事项。
...
请严格按照【注意事项】和【随症加减】的标题格式输出。"""
    ...
    def _call_combined_llm_judge(self, parsed_content: Dict[str, str], case: Dict[str, Any], skip_think: bool = False) -> Dict[str, float]:
        ...
        try:
            ...
            prompt_header = f"""\\no_think 你是一位资深中医评审专家，请根据病例描述，分别对以下{dimensions_count}个维度中待评估的内容按照各自的评分要点进行评分，并仅输出严格JSON。
...
{json_format}"""
    ...
    def evaluate_cause_mechanism(self, case: Dict[str, Any], model_interface, pbar: tqdm) -> Tuple[Dict[str, float], Dict[str, str]]:
        ...
        返回：(scores, responses)
        scores: {"病因": 0-1, "病机": 0-1}
        responses: 生成的病因/病机文本
        """
        try:
            # 1) 让被测模型生成病因/病机
    ...
    def _build_cause_mechanism_prompt(self, instruction: str) -> str:
        """构建用于被测模型生成病因/病机的提示词"""
        return f"""\\no_think 你是一位中医专家，请根据以下中医病例，分析其病因和病机。
...
"""
    ...
    def _call_cause_mechanism_judge(self, parsed: Dict[str, str], case: Dict[str, Any]) -> Dict[str, float]:
        """调用评分LLM对病因/病机进行0-100打分，并归一化到0-1"""
        gt_cause = case["output"].get("病因", "")
        gt_mechanism = case["output"].get("病机", "")
        prompt = f"""\\no_think 你是一位资深中医评审专家，请根据病例描述，结合各自的标准答案，分别对以下两个维度中的待评估内容按照各自的评分要点进行评分，并仅输出严格JSON：
...
{{
  ...
  "mechanism": 0-100
    ...
    def judge_prescription_by_reference(
        ...
        """
        使用评分LLM，基于奖励模型生成的参考答案，对待评测模型在4个维度的输出进行评分。

        Args:
            case: 原始病例
            syndrome_choice: 证型内容（已映射到选项内容）
            herbs_list_text: 处方组成（条目文本）
            references: 奖励模型参考答案 dict，键：compatibility/safety/incompatibility/pregnancy
            model_outputs: 待评测模型输出 dict，键同上
        Returns:
            维度分数字典（0-1）：{"方剂配伍规律", "安全性方面", "配伍禁忌", "妊娠禁忌"}
        """
        prompt = f"""\\no_think 你是一位资深中医评审专家，请根据病例描述、证型判断以及处方组成，结合各自的标准答案，分别对以下四个维度中的待评估内容按照各自的评分要点进行评分，并仅输出严格JSON：
