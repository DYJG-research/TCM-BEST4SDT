...
class LLMJudgeEvaluator:
    ...
    def __init__(self, api_host: str, api_port: int, model_name: str = "Qwen3-32B", api_key: Optional[str] = None):
        ...
        Args:
            api_host: API host
            api_port: API port
            model_name: Model name used for judging
        """
    ...
    def evaluate_all(self, case: Dict[str, Any], model_interface, pbar: tqdm, syndrome_choice: str = None, prescription_herbs: str = None, treatment_principles: str = None, skip_think: bool = False) -> Tuple[Dict[str, float], Dict[str, str]]:
        """
        Returns: (scores, responses)

        - scores: per-dimension scores
        - responses: raw generated text per dimension (for record keeping)
        - syndrome_choice: syndrome content for scoring "注意事项" and "随症加减" (fallback: case['output']['证型'])
        - prescription_herbs: prescription composition for scoring "煎服方法"/"注意事项"/"随症加减" (fallback: case['output']['药物组成及用量'])
        - treatment_principles: treatment principles used to generate "煎服方法" (fallback: case['output']['治则治法'])
        - skip_think: whether to skip judging "CoT内容完备性"
        """
        ...
        # 1) CoT completeness: only needs instruction (controlled by skip_think)
        if not skip_think:
            pbar.write("  生成CoT内容...")
        ...
        # 3) Generate precautions + syndrome modifications (combined)
        ...
        # Call the judge model to score dimensions
        pbar.write("  进行LLM评分...")
        scores = self._call_combined_llm_judge(parsed_content, case, skip_think=skip_think)
        
        # Build responses (format depends on skip_think)
        responses = {
        ...
        # Include CoT dimension only when not skipped
        if not skip_think:
    ...
    def _generate_think_content(self, case: Dict[str, Any], model_interface) -> str:
        """Generate content for judging CoT completeness."""
...
"""
    ...
    def _generate_precautions_and_modifications(self, case: Dict[str, Any], syndrome_choice: str, prescription_herbs: str, model_interface) -> Dict[str, str]:
        """Generate precautions and syndrome modifications in a single model call."""
        context_info = ""
        if syndrome_choice:
...
请严格按照【注意事项】和【随症加减】的标题格式输出。"""
    ...
    def _parse_precautions_and_modifications_response(self, response: str) -> Dict[str, str]:
        """Parse the combined response for precautions and syndrome modifications."""
    ...
    def _call_combined_llm_judge(self, parsed_content: Dict[str, str], case: Dict[str, Any], skip_think: bool = False) -> Dict[str, float]:
        """Score dimensions based on skip_think."""
        try:
...
"""
            ...
            # Include/exclude CoT completeness section based on skip_think
            dimension_sections = []
            dimension_counter = 1
...
{json_format}"""
            ...
            # Convert 0-100 to 0-1 (dimensions depend on skip_think)
            scores = {
            ...
            # Return default scores (dimensions depend on skip_think)
            error_scores = {
                "煎服方法": 0.0,
                "注意事项": 0.0,
                "随症加减": 0.0
    ...
    def _parse_json_response(self, response: str) -> Dict[str, Any]:
        ...
        result = {
            ...
            # Backward compatibility: cause/mechanism scoring keys
            "cause": 50,
            "mechanism": 50,
            # Backward compatibility: prescription 4-dimension scoring keys
            "compatibility": 50,
            "safety": 50,
            "incompatibility": 50,
            "pregnancy": 50
    ...
    def evaluate_cause_mechanism(self, case: Dict[str, Any], model_interface, pbar: tqdm) -> Tuple[Dict[str, float], Dict[str, str]]:
        """
        Generate cause/mechanism using the evaluated model, then score with the judge LLM against ground truth.
        """
        try:
            # 1) Have the evaluated model generate cause/mechanism
            ...
            # 2) Judge LLM scores against ground truth
            pbar.write("正在进行病因/病机的LLM评分...")
            scores = self._call_cause_mechanism_judge(parsed, case)

            # 3) Return scores and evaluated model outputs
            outputs = {
                "病因": parsed.get("病因", ""),
                "病机": parsed.get("病机", ""),
            }
...
{{
  ...
  "pregnancy": 0-100
    ...
    def evaluate_hallucination(self, case: Dict[str, Any], think_content: str, 
        ...
        """
        Evaluate hallucinations in CoT (CoT accuracy).
        
        Args:
            case: Case data
            think_content: Think content (already generated during CoT completeness; reused here)
            pbar: Progress bar
            
        Returns:
            (CoT accuracy score, hallucination details)
            - CoT accuracy score: 0-1, used for dimension_scores["CoT准确性"]
            - Hallucination details: full detection results, used for hallucination_details
        """
